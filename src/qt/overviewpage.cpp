...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFog_HillAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFog_HillAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFog_HillAmount = strAnonymizeFog_HillAmount.remove(strAnonymizeFog_HillAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFog_HillAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFog_HillAmount*COIN) nMaxToAnonymize = nAnonymizeFog_HillAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFog_HillAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFog_HillAmount));
        strAnonymizeFog_HillAmount = strAnonymizeFog_HillAmount.remove(strAnonymizeFog_HillAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFog_HillAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
