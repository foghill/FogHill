...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFog_HillAmount"))
        settings.setValue("nAnonymizeFog_HillAmount", 1000);
    nAnonymizeFog_HillAmount = settings.value("nAnonymizeFog_HillAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFog_HillAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeFog_HillAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFog_HillAmount:
            return QVariant(nAnonymizeFog_HillAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeFog_HillAmount:
            nAnonymizeFog_HillAmount = value.toInt();
            settings.setValue("nAnonymizeFog_HillAmount", nAnonymizeFog_HillAmount);
            emit AnonymizeFog_HillAmountChanged(nAnonymizeFog_HillAmount);
            break;
        default:
            break;
        }
