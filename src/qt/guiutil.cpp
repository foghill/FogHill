...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Fog Hill"))
        return false;
...
{
    // Convert Fog Hill:// to Fog_Hill:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Fog Hill://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Fog Hill:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fog Hill.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Fog_Hill.lnk
...
{
    return GetAutostartDir() / "Fog_Hill.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Fog_Hill.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Fog_Hill\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Fog_Hill app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Fog_Hill app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fog_Hill-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Fog_Hill-qt [" + tr("command-line options") + "]                     " + "\n";
