...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    else
    {
        ...
        {
            // Credit
            if (CFog_HillcoinAddress(rec->address).IsValid())
            {
                CTxDestination address = CFog_HillcoinAddress(rec->address).Get();
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
    {
        ...
        strHTML += "<b>" + tr("To") + ":</b> ";
        CTxDestination dest = CFog_HillcoinAddress(strAddress).Get();
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        ...
                        strHTML += GUIUtil::HtmlEscape(CFog_HillcoinAddress(address).ToString());
    ...
    if (wtx.IsCoinBase() || wtx.IsCoinStake())
    {
        strHTML += "<br>" + tr("Generated coins must mature 77 blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.") + "<br>";
    }
            ...
            if(txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    if (ExtractDestination(vout.scriptPubKey, address))
                    {
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                        strHTML += QString::fromStdString(CFog_HillcoinAddress(address).ToString());
                    }
