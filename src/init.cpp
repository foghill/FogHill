...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Fog_Hill.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Fog_Hilld.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeFog_Hillamount=<n> " + _("Keep N Fog_Hill anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Fog_Hill is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Fog_Hill is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Fog_Hill version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Fog_Hill server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Fog_Hill") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Fog_Hill to complete") << "\n";
    ...
    nAnonymizeFog_HillAmount = GetArg("-anonymizeFog_Hillamount", 0);
    if(nAnonymizeFog_HillAmount > 999999) nAnonymizeFog_HillAmount = 999999;
    if(nAnonymizeFog_HillAmount < 2) nAnonymizeFog_HillAmount = 2;
    ...
    LogPrintf("Anonymize Fog_Hill Amount %d\n", nAnonymizeFog_HillAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1FOG+1000 == (.1FOG+100)*10
       10FOG+10000 == (1FOG+1000)*10
    */
